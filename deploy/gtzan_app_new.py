...
N_VOTE      = 7   # segments used for fast classification
...
# ── PHASE 2: Full spectrogram (every segment, mel only) ──────────────────────
...
# ── Build seamless PNG ────────────────────────────────────────────────────────
_MAGMA = matplotlib.cm.get_cmap("magma")
...
try:
    ...
    # Single placeholder with exact fixed height — prevents any page reflow
    # Heights: 2 captions (~25px each) + iframe (263px) + audio player (70px) + gaps (~20px)
    SPEC_IFRAME_H  = STRIP_H + LABEL_H + 55   # 263px
    AUDIO_H        = 70
    CAPTION_H      = 25
    TOTAL_RESERVED = CAPTION_H * 2 + SPEC_IFRAME_H + AUDIO_H + 20  # 408px

    spec_placeholder  = st.empty()
    audio_placeholder = st.empty()

    # Skeleton: outer div locks the height, inner content is cosmetic
    with spec_placeholder.container():
        st.markdown(
            f'''<div style="
                min-height:{SPEC_IFRAME_H + CAPTION_H * 2}px;
                max-height:{SPEC_IFRAME_H + CAPTION_H * 2}px;
                overflow:hidden;
                background:#121218;
                border:1px solid #2a2a3a;
                border-radius:8px;
                display:flex; align-items:center; justify-content:center;
                color:#555; font-size:0.9rem; font-family:sans-serif;
            ">Spectrogram loading...</div>''',
            unsafe_allow_html=True,
        )
    with audio_placeholder.container():
        st.markdown(
            f'''<div style="min-height:{AUDIO_H}px; max-height:{AUDIO_H}px; overflow:hidden;"></div>''',
            unsafe_allow_html=True,
        )

    st.divider()

    # Top-3 + distribution — shown immediately, Phase 2 runs after
    ...
    with col_res:
        ...
        for rank, idx in enumerate(top_idx[:3]):
            ...
            medal = ["1.", "2.", "3."][rank]
            st.markdown(f"**{medal} {g.capitalize()}**")
    ...
    with col_dist:
        ...
        for idx in top_idx:
            ...
            with c1:
                st.progress(float(p), text=g)
            with c2:
                st.markdown(f"`{p*100:.1f}%`")

    # ── PHASE 2: Fill spectrogram placeholder ────────────────────────────────
    with spec_placeholder.container():
